                  ...
                  <article key={post._id} className="p-6 bg-white rounded-2xl shadow-2xl dark:bg-white relative pb-24 border border-solid border-cyan-400 border-x-4 border-y-4 pl-11 pr-11">
                      ...
                      <div className="absolute bottom-0 transform mb-5 m-2 ">
                          ...
                          {post.author === user._id ? (
                              <div>
                                  <button className="bg-cyan-400 text-white hover:bg-cyan-500 rounded-lg text-sm px-4 py-2 text-center font-semibold w-32 h-14 m-4" onClick={() => toggleUpdateBlog(post._id)}>UPDATE</button>
                                  <button className="bg-cyan-400 text-white hover:bg-cyan-500 rounded-lg text-sm px-4 py-2 text-center font-semibold w-32 h-14 m-4">DELETE</button>
                              </div>
                          ) : null}
          
          
                      </div>
                      {updateBlogs[post._id] && X && (
                          <UpdateBlogs post={post} setX={setX(true)} setUpdateBlog={toggleUpdateBlog} />
                      )}
                  </article>
              )
          ))}
